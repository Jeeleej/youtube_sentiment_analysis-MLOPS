...
def train_stacking_model(X_train, y_train, learning_rate: float, max_depth: int, n_estimators: int) -> StackingClassifier:
    ...
    try:
        ...
        logger.debug('Stacking Classifier training completed')
        return model
    except Exception as e:
        logger.error('Error during Stacking Classifier training: %s', e)
        raise
...
def main():
    try:
        ...
        best_model = train_stacking_model(X_train_tfidf, y_train, learning_rate, max_depth, n_estimators)

        # Save the trained stacking model
        save_model(best_model, os.path.join(root_dir, 'stacking_model.pkl'))
